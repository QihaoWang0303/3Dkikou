...
#define	MOUSESPD			(0.002f)		//マオスのスピード
...
void UpdateCamera(void)
{
	...
	float offsetX = (mousePos.x - centerX) * MOUSESPD;
	float offsetY = (mousePos.y - centerY) * MOUSESPD;
...
void SetViewPort(int type)
{
	...
	case TYPE_DOWNLEFT_HALF_SCREEN:
		vp.Width = (FLOAT)SCREEN_WIDTH / 2;
		vp.Height = (FLOAT)SCREEN_HEIGHT / 2;
		vp.MinDepth = 0.0f;
		vp.MaxDepth = 1.0f;
	...
	case TYPE_DOWNRIGHT_HALF_SCREEN:
		vp.Width = (FLOAT)SCREEN_WIDTH/2;
		vp.Height = (FLOAT)SCREEN_HEIGHT / 2;
		vp.MinDepth = 0.0f;
		vp.MaxDepth = 1.0f;
